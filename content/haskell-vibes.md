...
<iframe width="560" height="315" src="https://www.youtube.com/embed/qMQ-y9dHE2k?si=sqAwlSOezHOboQ8n" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
...
So Trapped it in [a container](https://github.com/jappeace/haskell-vibes).
I asked it to implement some front-end map features,
it did well.
Now I wondered if it could do backend development,
Haskell type errors are a lot more difficult then
...
[^llm]: Read ChatGPT if you're not familiar with this term. 
        ...
        The other thing claude code does is keep on churning untill 
...
[^dreadful]: Logging in to claude was dreadful.
    Jesus why does the terminal break in docker 
    containers of certain window sizes?
    Claude service went down as well several times.
    Which coincided me trying to use it.
    I bet they used claude to solve the outage,
    it even had the same status updates which you get from the console.
    I'm sure they had people solving the issue but their service is so buggy. 
    In another event I managed to log into my company's email, 
    asside the company's enterprise account. 
    So there are now 2 accounts linked to the company. Even though I payed for the company email one.
...
it's just a 
fair bit investigation, and solving many compile errors.
It did so [with no problems](https://github.com/jappeace/ram/commit/b0e2e66ccc8537c143acb9caf749ef751a1c047f).

[^discussion]: It's part of a larger discussion on what to do with basement: https://discourse.haskell.org/t/fork-basement-as-baseplate/12415/82
...
The type has align with the expression (aka check) or else you don't get a program but an error instead.
...
[^hold-beyond]: Although I don't hold it beyond it, at several times it claimed it wasn't able to enter the nix-shell. Which is a flatout lie. It just doesn't want to build.

[^compression]: I think a problem is "context compression" also compressing the CLAUDE.md file, I just told it to reread it if it's doing compression, that appears to keep it in context. (this could be another bug?)
...
I review it's code to verify if it's correct after it's done and overcame all these hurdles.
I don't want to babysit it.
The container protects me sufficiently from it's weirdness.
There is weirdness. 
At one point it tried writing in `/etc/shadow` to give itself write access to the 
home folder, which I had forgotten it to give.
...
it just takes me a long time and it's mostly kind of boring work.
I recognize Claude is much faster at grinding through the compile errors then I am,
...
## Job
What is my job?
Do my employers care about me specifically implementing the software?
Do they care about me being the one solving the compile errors?
No.
They want a reliable piece of software.
But I used to spend the majority of my time writing it 
because there was no other way to write it.
This may be confuse you into believing writing software is your job,
...
I finished friday with so many PR's open and I flagged this as a concern.
It's not a concern. 
...
I've to ensure the software Claude produces works.
I've to think in terms of verification.
...
I would give her access to my machine and without worries.

I realize this is what my employers want as well.
Trust the job gets done.
How isn't important.
It's almost as if automating lower value jobs 
creates opportunities for more higher value jobs. [^doing]
