...
[^katip]: Katip is great btw, structured logging is really nice, 
          ...
          it denies the existence of hetrogenous lists in java which haskell tuple's get encoded to as JSON.
...
The website became a lot more performant "magically", [^read-between-lines]
...
I did expectation management around rolling out features fast,
...
Unlike a normal web app we also have sensors and a pipeline of continuous flowing data.
...
But they were seeing movement, and this caused a renew.
...
They just send the hardware to a factory and told them to figure it out. 😅
You can maybe do this with a mature known product.
But this was a barely functioning prototype from a nobody company
...
[^engineering-issues]: Rather then streaming the signals and doing trilateration on the fly, these were processed as a batch.
                       I don't thin anyone at the time had expected live updates to be a requirement.
...
I think around May in 2024 the money had ran out for this project.
so the other engineer got pulled,
which was annoying because she just had gotten reasonably productive
to do small fullstack features on her own.
...
[^no-symtypes]: My biggest issue is the lack of sumtypes which make error hadnling hard. 
                You get enums but no hetrogenic struct per tag.
...
and he was going to, with the minimum of efforts.
But bob seemed to be delighted to share about danish culture and
cuisine, what a joyful reminiscence.
The check involved production people would attaching a bunch
...
Also because the way this factory was managed was extremly strange
and inefficient.
...
and nobody had any clue they were using these tools like this.
We did not know this either, 
but we had scheduled several days preparing for this check,
and at some point I started asking the PM about what was going on,
which made him think about it.
...
I stuck around with because I liked the people,
may in fact also be helping the environment? [^environment]

[^environment]: Now, I have no aspirations to actually help the environment, because I feel this is mostly an energy issue. But I am definelty concerned about it. So I count this as a bonus.
...
In fact on the road to this check the PM was was already casually
...
On judgement day, before the check was being done by Bob.
...
Find a tag, press the sensor too it, wait a second, move on.[^firmware-bugs]
...
in my bag, 
we'd just walk with them on person while doing the check.
...
We past the check by the skin of our teeth.
...
The algo engineer and I would share responsibilities.
...
but this company can build it's own hardware.
...
modifications, 
for example, a sleep mode where we disable the sensors on off hours.
I've never claimed to be good at firmware,
it's just that I'm the lemons we got.
Despite how cooked internal politics is for this company.
We're winning this game.

[^read-between-lines]: I did nothing "genius" of course, I just made a boring traditional design out of it. The reason it was so fried is because the previous tech lead was affraid of Haskell.
