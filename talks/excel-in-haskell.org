...
submissionSpec = withApp $ do
  describe "roundtrip download all zip -> submission" $ it "should be able to re-submit a pack archive as a Zip-of-Xlsx submission " $ do
    ...
    rows <- genFake $ runConduit $
              (do
                  fakePremiumEstimates 3 .| CC.map (\x -> x{policyBordereauSubmissionId=pesId, policyBordereauSubsectionId=subsectId1})
              .| sinkList

    runDB $ do
      forM_ rows $ \r -> do
        uuid <- genKey PolicyBordereauKey
        P.insertKey uuid r
    withSystemTempDirectory "DownloadAllSpec_Result" $ \tmpDir -> do
        ...
        rs <- runDB $ P.selectList [PolicyBordereauSubmissionId P.==. pesId] sortCols
        ...
        rs2 <- runDB $ P.selectList [PolicyBordereauSubmissionId P.==. pesId2] sortCols
        ...
        let asIfSameSubmission r = r {policyBordereauSubmissionId = pesId}
        assertEq "rows match"
